...
public class PackageBuilder {
    ...
    public static void main(String[] args) throws RemoteException, Exception {
        ...
        // if (args.length < 2) {
        // System.out.println("Usage: java -jar PackageBuilder.jar <org property
        // file path> <fetch property path> [metadataitems=item1,item2,...]
        // [skipitems=skipPattern1,skipPatter2,...]");
        // System.out.println("properties\test.properties
        // properties\fetch.properties - will list the items defined in the
        // fetch.properties file from the org specified in the file
        // properties\test.properties and put them in the target directory
        // specified in the properties\fetch.properties file ");
        // System.out.println("providing metadataitems and/or skipitems
        // parameters on command line will override the same from the fetch
        // properties file");
        ...
        for (String itemType : actualInventory.keySet()) {
            ...
            for (InventoryItem oneMetaData : itemList) {
                ...
                if (downLoadedFileName != null) {
                    ...
                    Set<InventoryItem> curItems = (result.containsKey(curEmail)) ? result.get(curEmail)
                            : new HashSet<>();
    ...
    private String downloadIfChanged(String itemType, InventoryItem oneMetaData) throws IOException {
    ...
    private void generateInventoryFromOrg(HashMap<String, ArrayList<InventoryItem>> inventory)
        ...
        while (i.hasNext()) {
            ...
            if (loglevel.getLevel() > Loglevel.BRIEF.getLevel()) {
                ...
                logPartialLine("Processing type " + counter + " out of " + workToDo.size() + ": " + mdType,
                        Loglevel.BRIEF);
            }
    ...
    private void generateInventoryFromDir(HashMap<String, ArrayList<InventoryItem>> inventory) throws IOException {
        ...
        if (!Utils.checkIsDirectory(basedir)) {
            ...
            log("Base directory parameter provided: " + basedir + " invalid or is not a directory, cannot continue.",
                    Loglevel.BRIEF);
            System.exit(1);
        }
        ...
        for (String s : filelist) {
            ...
            try {
                ...
                if (mdType == null) {
                    log("Couldn't find type mapping for item : " + mdType + " : " + filename + ", original path: " + s
                ...
                if (filename.contains("/") && mdType.equals("AuraDefinitionBundle")) {
                    ...
                    log("Added: " + mdType + " : " + subFoldername + ", to inventory, original path: " + s,
                            Loglevel.NORMAL);
                    continue;
                }
    ...
    private HashMap<String, ArrayList<InventoryItem>> generatePackageXML(
        ...
        log("Total items skipped: " + skipCount + " (excludes count of items in type where entire type was skipped)",
                Loglevel.NORMAL);
    ...
    private HashMap<String, ArrayList<InventoryItem>>[] breakPackageIntoFiles(
            HashMap<String, ArrayList<InventoryItem>> myFile) {
        ...
        for (String mdType : myFile.keySet()) {
            ...
            log("Adding type: " + mdType + "(" + mdTypeSize + " items) to file " + fileIndex + ", total count now: "
                    + fileCount, Loglevel.NORMAL);
        }
        ...
        HashMap<String, ArrayList<InventoryItem>>[] retval = (HashMap<String, ArrayList<InventoryItem>>[]) new HashMap[files
                .size()];
    ...
    private int handleSkippingItems(HashMap<String, ArrayList<InventoryItem>> myFile) {
        ...
        for (String mdType : myFile.keySet()) {
            ...
            for (Pattern p : skipPatterns) {
                ...
                if (m.matches()) {
                    log("Skip pattern: " + p.pattern() + " matches the metadata type: " + mdTypeFullName
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                for (Pattern p : skipPatterns) {
                    Matcher m = p.matcher(mdTypeFullName);
                    if (m.matches()) {
                        log("Skip pattern: " + p.pattern() + " matches the metadata item: " + mdTypeFullName
    ...
    private HashMap<String, InventoryItem> fetchMetadataType(String metadataType) throws RemoteException, Exception {
        ...
        try {
            ...
            if (obj != null && obj.getInFolder() == true) {
                ...
                query.setType(type);
                FileProperties[] srcMd = srcMetadataConnection.listMetadata(new ListMetadataQuery[] { query },
                        myApiVersion);
            ...
            do {
                ...
                if (folderName != null) {
                    log("Processing folder: " + folderName + " " + " items: " + srcMd.length + "\tCurrent total: "
    ...
    private void addParameterFromProperty(final Properties props, final String propName) {
        ...
        String paramName = (propName.startsWith("sf.")) ? propName.substring(3) : propName;
